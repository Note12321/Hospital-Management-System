...
def func():
    try:
        return render_template('func.html', name=session.get('name'))
...
def generate_report(data):
    ...
    # 上传信息
    upload_info = Paragraph(f"<font size=12><b>Filename:</b> {filename}<br/><b>Upload Time:</b> {upload_time}</font>",
                            styles['Normal'])
...
if __name__ == '__main__':
    ...
    app.run(host='127.0.0.1',port=1142)
